          ...
          {seg === 'crews' && (
            <>
              ...
              {circleData.friends.length > 0 ? (
                ...
                <div style={{ marginTop: '20px', padding: '30px 24px', borderRadius: '16px', border: `1px solid ${HAIR_HI}`, background: 'linear-gradient(150deg, rgba(232,233,237,.04), rgba(232,233,237,.01))', textAlign: 'center' }}>
                  <p style={{ fontFamily: 'DM Sans', fontSize: '13px', color: BONE_MID, lineHeight: 1.65, margin: 0, maxWidth: '300px', display: 'inline-block' }}>
                    a crew is made of your people — add amigos from their worlds first.
                  </p>
                  <div>
                    <button className="pressable" onClick={() => navigate('/community')} style={{ marginTop: '16px', display: 'inline-flex', alignItems: 'center', gap: '8px', background: BONE, border: 'none', borderRadius: '11px', padding: '13px 24px', color: VOID, fontFamily: 'DM Sans', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                      Find your people <ArrowUpRight size={16} />
                    </button>
                  </div>
                </div>
...
/* YOUR CIRCLE — requests waiting on you, then one quiet honest count.
   The circle is intimate: this number shows only to you, nowhere public. */
function CircleBlock({ circle, busyId, onAnswer }) {
  const { friends, pending_in } = circle
  if (!pending_in.length && !friends.length) return null
  return (
    <div style={{ marginTop: '18px' }}>
      <div style={{ fontFamily: 'DM Mono', fontSize: '9px', color: BONE_LOW, letterSpacing: '.3em', textTransform: 'uppercase' }}>○ your circle</div>
      {pending_in.map((p) => {
      ...
      {friends.length > 0 && (
        <div data-testid="circle-count" style={{ fontFamily: 'DM Mono', fontSize: '9px', color: BONE_LOW, letterSpacing: '.14em', marginTop: '10px' }}>
...
function PlanSheet({ friends, onClose, onCreated }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      await createPlan({ title, spot, detail, startsAt, inviteeIds: [...sel] })
      onCreated()
