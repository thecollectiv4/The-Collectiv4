...
export default function Community() {
  ...
  if (!open) {
    const ticketCard = (
      <div style={{width:'100%',maxWidth:'360px',borderRadius:'20px',overflow:'hidden',background:'var(--bg-card)',border:'1px solid var(--border-hi)',boxShadow:'0 8px 40px rgba(0,0,0,.4)',transition:'all .3s',cursor:hasTicket?'pointer':'default'}}
        onClick={()=>hasTicket&&setOpen(true)}
        onMouseOver={e=>{if(hasTicket){e.currentTarget.style.transform='translateY(-4px)';e.currentTarget.style.boxShadow='0 16px 50px rgba(0,0,0,.5)'}}}
        onMouseOut={e=>{if(hasTicket){e.currentTarget.style.transform='translateY(0)';e.currentTarget.style.boxShadow='0 8px 40px rgba(0,0,0,.4)'}}}>
        ...
        <div style={{padding:'20px 28px',display:'flex',justifyContent:'space-around'}}>
          {[['MAY 30','DATE'],['10PM','DOORS'],['HTX','CITY']].map(([v,l],i)=>(
        ...
        <div style={{padding:'16px 28px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
            <Users size={12} style={{color:'var(--cream-low)'}}/><span style={{fontFamily:'DM Mono',fontSize:'10px',color:'var(--cream-low)'}}>{attendees.length} going</span>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
    ...
    return (
      <div style={{background:'linear-gradient(180deg,#0E0D0C 0%,#0A0908 40%,#0A0908 100%)',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'28px',overflow:'hidden',position:'fixed',inset:0,maxWidth:'430px',margin:'0 auto'}}>
        {!hasTicket ? (
          <div style={{position:'relative',width:'100%',maxWidth:'360px'}}>
            <div style={{position:'absolute',inset:0,zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'20px',borderRadius:'20px'}}>
              <div style={{fontFamily:'Bebas Neue',fontSize:'32px',color:'#FFFFFF',textAlign:'center',textShadow:'0 2px 20px rgba(0,0,0,.8)'}}>GET YOUR TICKET<br/>TO UNLOCK</div>
              <button onClick={()=>navigate('/')} style={{background:'var(--cream)',border:'none',borderRadius:'10px',padding:'14px 36px',color:'var(--bg)',fontFamily:'DM Sans',fontSize:'13px',fontWeight:600,cursor:'pointer',boxShadow:'0 4px 20px rgba(0,0,0,.4)'}}>Get Ticket</button>
            </div>
            <div style={{filter:'blur(6px)',opacity:0.35,pointerEvents:'none'}}>{ticketCard}</div>
          </div>
        ) : ticketCard}
      </div>
    )
    ...
    <div style={{background:'linear-gradient(180deg,#0E0D0C 0%,#0A0908 40%,#0A0908 100%)',minHeight:'100vh',display:'flex',flexDirection:'column'}}>
      ...
      {tab==='going'?(
        <div style={{padding:'12px 28px 100px',flex:1}}>
          {loading?<div style={{textAlign:'center',padding:'40px',color:'var(--cream-low)'}}>Loading...</div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column'}}>
          ...
          <div style={{position:'fixed',bottom:'72px',left:0,right:0,background:'rgba(10,9,8,.95)',borderTop:'1px solid var(--border)',padding:'10px 20px',zIndex:99}}>
            <div style={{maxWidth:'430px',margin:'0 auto',display:'flex',gap:'8px',alignItems:'center'}}>
              ...
              <button onClick={send} disabled={!newMsg.trim()} style={{width:'40px',height:'40px',borderRadius:'50%',background:newMsg.trim()?'var(--cream)':'var(--bg-raised)',border:'none',cursor:newMsg.trim()?'pointer':'default',display:'flex',alignItems:'center',justifyContent:'center'}}>
