...
export default function UserProfile() {
  ...
  if (!profile) return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '18px', background: 'var(--bg)', padding: '40px' }}>
      ...
      <button onClick={() => navigate('/community')} style={{ background: 'rgba(var(--ink-rgb),.06)', border: '1px solid var(--border-hi)', borderRadius: '8px', padding: '9px 18px', color: 'var(--cream-mid)', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans' }}>Back to Community</button>
    </div>
  )

  const topBar = (
    <>
      <button onClick={() => navigate(-1)} aria-label="Back" style={{ background: 'rgba(var(--void-rgb),.6)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', border: '1px solid rgba(var(--ink-rgb),.18)', borderRadius: '100px', width: '34px', height: '34px', color: 'var(--cream)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ArrowLeft size={16} /></button>
      <span />
    </>
  )
