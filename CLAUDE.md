...
5. **The hard rules don't bend for convenience.** Restated because these are the ones a helpful agent talks itself past:
   ...
   - **The payment machine has an automatic proof — run it when the change touches money** (checkout, webhook, `/claim`, auth `?next`, or `api/` itself): `PREVIEW_URL=<preview> STRIPE_WEBHOOK_SECRET=<preview> SUPABASE_SERVICE_KEY=<preview> SUPABASE_URL=… npx playwright test e2e/walkthrough-purchase.spec.js` (v19). It rides intent→checkout(200 + real Stripe test URL)→a validly-SIGNED webhook→a real ticket in the DB owned by the buyer→idempotency→`/claim`, then purges its QA and asserts zero residual. **Preview only** (prod's Stripe key is LIVE). The real hosted card page + Stripe's own webhook delivery are out of scope (no Stripe CLI in CI) and simulated with a valid signature — that boundary is documented in the spec header, don't paper over it.
